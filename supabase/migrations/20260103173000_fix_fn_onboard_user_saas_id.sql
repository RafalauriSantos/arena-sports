-- Fix fn_onboard_user: tenants.saas_id is UUID FK to saas_products.id
--
-- Root cause:
-- - Frontend fallback and the initial RPC implementation were passing a slug (text)
--   into tenants.saas_id (uuid), causing PostgREST 400 errors and leaving
--   profiles.tenant_id null.
