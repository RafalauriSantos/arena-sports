-- Relax fn_onboard_user SaaS lookup
--
-- Problem:
-- - fn_onboard_user was filtering saas_products.active = true when the column exists.
--   If the row exists but is inactive, onboarding fails with 400 and leaves profiles.tenant_id null.
--
-- Fix:
-- - Resolve saas_products by slug regardless of active, preferring active rows when possible.
