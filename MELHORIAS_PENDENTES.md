# Melhorias Pendentes (espelho do Notion)

Ultima atualizacao: 2026-04-04
Fonte: Notion > ArenaSys - Backlog de Produto > Database "Melhorias"

Regra deste arquivo:

- Somente melhorias ainda nao concluidas.
- Ordem segue o campo "Ordem" do Notion.
- Foco em execucao incremental (fazendo aos poucos).

## Fase 1 - Prioridade imediata (Ordem 1 a 4)

- [ ] [#1] Visualizacao de agenda por dia / semana / mes
  - Modulo: Agenda / Visualizacao
  - Dificuldade: Facil
  - Situacao atual: Parcialmente implementado
  - Link Notion: https://www.notion.so/3362113a244881908cbcdc1e85dc1413

- [ ] [#2] Check-in do jogador na chegada
  - Modulo: Check-in / Operacao
  - Dificuldade: Medio
  - Situacao atual: Nao existe
  - Link Notion: https://www.notion.so/3362113a2448812db00ce34c148871df

- [ ] [#3] Controle de day use (entrada avulsa sem reserva)
  - Modulo: Check-in / Operacao
  - Dificuldade: Medio
  - Situacao atual: Nao existe
  - Link Notion: https://www.notion.so/3362113a244881d6b138f284689658c2

- [ ] [#4] Conciliacao fim de dia: agenda vs realidade
  - Modulo: Check-in / Operacao
  - Dificuldade: Medio
  - Situacao atual: Nao existe
  - Link Notion: https://www.notion.so/3362113a24488120a80dd0d962c07574

## Fase 2 - Receita e operacao financeira (Ordem 5 a 8)

- [ ] [#5] Pagamento no ato do agendamento via link publico
  - Modulo: Financeiro
  - Dificuldade: Medio
  - Situacao atual: Nao existe
  - Link Notion: https://www.notion.so/3362113a24488194a16cd417c0b54ec7

- [ ] [#6] Timeout de reserva: liberar horario se nao pagar em X minutos
  - Modulo: Financeiro
  - Dificuldade: Medio
  - Situacao atual: Nao existe
  - Link Notion: https://www.notion.so/3362113a2448810fad1fe519bd5d0077

- [ ] [#7] Politica de cancelamento e estorno configuravel por arena
  - Modulo: Financeiro
  - Dificuldade: Medio
  - Situacao atual: Nao existe
  - Link Notion: https://www.notion.so/3362113a244881c3b9dde8d61c28df50

- [ ] [#8] Notificacao de estorno processado para o jogador
  - Modulo: Financeiro
  - Dificuldade: Facil
  - Situacao atual: Nao existe
  - Link Notion: https://www.notion.so/3362113a2448810f905cd7e517487514

## Fase 3 - Inteligencia e analiticos (Ordem 9 a 13)

- [ ] [#9] Relatorio de horarios mais reservados
  - Modulo: Dashboard / Relatorios
  - Dificuldade: Medio
  - Situacao atual: Parcialmente implementado
  - Link Notion: https://www.notion.so/3362113a244881c88defc812615cf20f

- [ ] [#10] Breakdown por genero nos relatorios
  - Modulo: Dashboard / Relatorios
  - Dificuldade: Medio
  - Situacao atual: Nao existe
  - Link Notion: https://www.notion.so/3362113a244881d0aa76dca16cded899

- [ ] [#11] Dashboard financeiro: caixa, estornos e visao geral
  - Modulo: Financeiro
  - Dificuldade: Dificil
  - Situacao atual: Parcialmente implementado
  - Link Notion: https://www.notion.so/3362113a2448817e95def2d15b8ac45f

- [ ] [#12] Mapa de calor de ocupacao das quadras
  - Modulo: Dashboard / Relatorios
  - Dificuldade: Dificil
  - Situacao atual: Nao existe
  - Link Notion: https://www.notion.so/3362113a24488113beebc45feb2fe705

- [ ] [#13] Dashboard avancado com KPIs de gestao
  - Modulo: Dashboard / Relatorios
  - Dificuldade: Dificil
  - Situacao atual: Parcialmente implementado
  - Link Notion: https://www.notion.so/3362113a244881748badf49f9e4faf88

## Como vamos executar aos poucos

1. Sempre puxar o proximo item nao concluido da Fase 1.
2. Implementar, testar e subir para producao.
3. Marcar o checkbox local e atualizar o status no Notion.
4. Passar para o proximo item da ordem.
