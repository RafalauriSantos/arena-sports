...
import { CheckCircle, Copy, Users, Plus, ArrowLeft, Share2, MessageCircle, Crown, QrCode, Download, Clock, CheckCircle2, Ticket } from "lucide-react";
...
export function SuccessScreen({ booking, onBack, onUpdatePlayers }: SuccessScreenProps) {
  ...
  const handlePlayerPayment = (playerName: string) => setSelectedPlayer(playerName);
        ...
        <Card className="w-full max-w-sm bg-primary p-6 text-primary-foreground rounded-2xl">
          <div className="text-center space-y-4">
            ...
            <div className="space-y-2 py-4 border-y border-primary-foreground/30">
              ...
              <p className="text-2xl font-bold mt-2 number-display">R$ {pricePerPlayer.toFixed(2).replace('.', ',')}</p>
            </div>
            <p className="text-xs opacity-60">Validado pelo {ARENA_CONFIG.name}</p>
            <div className="flex gap-2 pt-4">
              <Button variant="secondary" className="flex-1 btn-press" onClick={() => setShowReceipt(null)}>
                Fechar
              </Button>
  ...
  if (selectedPlayer) {
    return (
      <div className="min-h-screen bg-background p-4 animate-fade-in">
        <Button variant="ghost" className="mb-4 text-muted-foreground btn-press" onClick={() => setSelectedPlayer(null)}>
        ...
        <Card className="bg-card border-border p-6 text-center space-y-6 rounded-2xl">
          ...
          <div className="py-6 border-y border-border">
            <p className="text-3xl font-black text-primary number-display">R$ {pricePerPlayer.toFixed(2).replace('.', ',')}</p>
          ...
          <Button className="w-full bg-primary text-primary-foreground hover:bg-primary/90 h-14 text-lg btn-press" onClick={handleConfirmSentPix}>
    ...
    <div className="min-h-screen bg-background animate-fade-in">
      ...
      <div className="sticky top-0 z-50 bg-background/80 backdrop-blur-lg border-b border-border">
        <div className="container py-4">
          <button onClick={onBack} className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors btn-press">
      ...
      <div className="container px-4 py-4 md:py-6 space-y-4 md:space-y-6 pb-28 md:pb-6">
        ...
        <Card className="relative overflow-hidden bg-gradient-to-br from-primary/20 via-card to-card border-primary/30 p-6 rounded-2xl">
          ...
          <div className="text-center space-y-4">
            ...
            <div>
              <h1 className="text-2xl font-black text-foreground">Jogo Confirmado!</h1>
              <p className="text-muted-foreground mt-1">
                {booking.paymentType === "pix" ? "✓ Pagamento confirmado" : "Aguardando aprovação"}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-primary">{booking.fieldName}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-sm font-medium text-foreground">{booking.bookedBy}</p>
              </div>
            </div>
          </div>
        </Card>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-muted-foreground text-xs">Receba dos jogadores</p>
              </div>
            </div>
            <Switch checked={vaquinhaEnabled} onCheckedChange={setVaquinhaEnabled} />
          </div>

          {vaquinhaEnabled && (
            <div className="space-y-4 pt-4 border-t border-border animate-fade-in">
              <div>
                <label className="text-sm text-muted-foreground mb-2 block">Sua Chave Pix</label>
                <Input
              ...
              <Button variant="outline" className="w-full border-primary text-primary hover:bg-primary/10 btn-press" onClick={handleCopyPaymentLink}>
        ...
        {vaquinhaEnabled && players.length > 0 && (
          <Card className="p-5 rounded-2xl bg-card border border-border space-y-3">
            ...
            {progressPercent >= 100 && (
              <Button className="w-full mt-2 bg-primary text-primary-foreground hover:bg-primary/90 h-12 btn-press" onClick={handlePayArena}>
                <CheckCircle className="h-5 w-5 mr-2" />
                Pagar Arena Agora
              </Button>
            )}
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className={cn(
              ...
              {remainingPlayers > 0 ? `Faltam ${remainingPlayers}` : "Completo!"}
            </div>
          </div>
          ...
          {remainingPlayers > 0 && (
            <div className="flex gap-2">
              ...
              />
              <Button onClick={handleAddPlayer} disabled={!newPlayer.trim()} className="bg-primary hover:bg-primary/90 text-primary-foreground btn-press">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {Array.from({ length: booking.totalPlayers }).map((_, index) => {
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="text-lg font-bold number-display">{index + 1}</span>
                        <span className="text-sm font-medium truncate max-w-20">{player.name}</span>
                      </div>
                      {player.status === "paid" && <CheckCircle2 className="h-4 w-4" />}
                      {player.status === "awaiting" && <Clock className="h-4 w-4" />}
                    </div>
                    
                    {vaquinhaEnabled && (
                      <div className="mt-2">
                        {player.status === "pending" && (
                          <Button size="sm" variant="outline" className="w-full h-7 text-xs" onClick={() => handlePlayerPayment(player.name)}>
                            Pagar
                          </Button>
                        )}
                        {player.status === "awaiting" && (
                          <Button size="sm" className="w-full h-7 text-xs bg-warning text-warning-foreground" onClick={() => handleConfirmReceived(player.name)}>
                            Confirmar
                          </Button>
                        )}
                        {player.status === "paid" && (
                          <Button size="sm" variant="ghost" className="w-full h-7 text-xs" onClick={() => handleShowReceipt(player.name)}>
                            Ver Recibo
                          </Button>
                        )}
                      </div>
              ...
              return (
                <Card key={index} className="p-3 rounded-xl border-2 border-dashed border-border bg-card/50">
