...
export function AdminDashboardNew({
	...
	return (
		<div className="bg-background pb-28 md:pb-24 px-4 md:px-0">
					...
					<div className="space-y-3">
						{daySlots.map((slot) => (
							<AdminSlotCard
								...
								onViewDetails={() =>
									slot.booking && handleViewDetails(slot.booking)
								}
								onMarkAsPaid={() =>
									slot.booking && onMarkAsPaid?.(slot.booking.id)
								}
								onCancel={() =>
									slot.booking && onCancelBooking?.(slot.booking.id)
								}
							/>
						))}
					</div>
