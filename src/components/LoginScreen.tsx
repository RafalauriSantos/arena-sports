...
const SAVED_PHONE_KEY = "arena_saved_phone";
...
export function LoginScreen({ onLogin }: LoginScreenProps) {
	...
	useEffect(() => {
		try {
			...
			if (saved) {
				// We expect saved to be digits-only (unformatted)
				const formatted = formatPhone(saved);
				setPhone(formatted);
				setRemember(true);
			}
		} catch (e) {
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		if (numbers.length >= 10) {
			...
			try {
				if (remember) {
					localStorage.setItem(SAVED_PHONE_KEY, numbers);
				} else {
