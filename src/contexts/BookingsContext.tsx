...
export function BookingsProvider({ children }: { children: ReactNode }) {
	...
	const connectionTenantRef = useRef<string | null>(null);
	...
	useEffect(() => {
		...
		const setup = async () => {
			...
			// Evita reconectar se já estamos conectados ao mesmo tenant
			if (
				isConnectedRef.current &&
				connectionTenantRef.current === currentTenantId
			) {
				console.log("🔄 [REALTIME] Já conectado ao tenant:", currentTenantId);
				return;
			}

			// Log apenas na primeira conexão ou troca de tenant
			console.log(
				"📡 [REALTIME] Conectando canal para tenant:",
				currentTenantId,
			);
			...
			const channel = supabase
				...
				.subscribe((status, err) => {
					if (status === "SUBSCRIBED") {
						console.log(
							"✅ [REALTIME] Conectado! Escutando eventos de bookings e courts",
						);
		...
		return () => {
			...
			if (realtimeChannelRef.current) {
				...
				realtimeChannelRef.current = null;
				isConnectedRef.current = false;
				connectionTenantRef.current = null;
			}
		};
